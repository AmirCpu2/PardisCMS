...
#pragma warning restore 0219
    ...
    public class JqGridSearch
    {
        ...
        public IQueryable<T> ApplyFilter<T>(IQueryable<T> query, bool _search, string searchField, string searchString,
            string searchOper, string filters, NameValueCollection form, string searchFieldCustom = null)
        {
            if (!_search && searchFieldCustom == null)
                return query;
            ...
            if (!string.IsNullOrWhiteSpace(searchString) &&
                ...
                !string.IsNullOrWhiteSpace(searchFieldCustom)
                )
            {
                if (searchField?.Length > 40)
                    searchField = searchField?.Substring(0, 40);
                if (searchString?.Length > 60)
                    searchString = searchString?.Substring(0, 60);
                if (searchFieldCustom?.Length > 200)
                    searchFieldCustom = searchFieldCustom?.Substring(0, 200);
                ...
                searchFieldCustom = regexForSearchField.Replace(searchFieldCustom ?? "", "");
                ...
                query = ManageCustomFilter(query, searchFieldCustom);
                if (!_search)
                    return query;

                query = manageSingleFieldSearch(query, searchField, searchString, searchOper);
            }
        ...
        private static IQueryable<T> ManageCustomFilter<T>(IQueryable<T> inputQuery, string customFilters)
        {
            if (string.IsNullOrEmpty(customFilters))
                return inputQuery;

            var list = customFilters.Split(',');
            ...
            foreach (var item in list)
            {
                ...
                if (string.IsNullOrEmpty(value) || value == "0" || value == "undefined")
                    continue;
                inputQuery = FilterObjectSet<T>(inputQuery, key, value);
            }
            return inputQuery;
        }
        public static IQueryable<T> FilterObjectSet<T>(IQueryable<T> inputQuery, string FieldName, string data)
        {
            var par = Expression.Parameter(typeof(T));

            var prop = Expression.PropertyOrField(par, FieldName);
            ...
            if (propType.IsGenericType && propType.GetGenericTypeDefinition() == typeof(Nullable<>))
            {
                if (String.IsNullOrEmpty(data))
                    ...
                    data2 = Convert.ChangeType(data, propType.GetGenericArguments()[0]);
            }
            else
            {
                data2 = Convert.ChangeType(data, propType);
            }
            ...
            if (propType == typeof(string))
            {
                ...
                var someValue = Expression.Constant(data2, typeof(string));
            ...
            {
                var eq = Expression.Equal(prop, Expression.Convert(Expression.Constant(data2), prop.Type));
    ...
    public static class ReflectionHelper
    {
        public static Type FindFieldType(this Type type, string fieldName, string parent = "", int dumpLevel = 3)
        {
            ...
            foreach (var property in properties)
            {
                ...
                if (isNestedProperty(property.PropertyType) && type != property.PropertyType)
                {
